...
# def base_schema():
#     return Schema(
#         NestedField(field_id=1, name="number_partitioned", field_type=LongType(), required=False),
#         NestedField(field_id=2, name="primary_key", field_type=StringType(), required=False),
#         NestedField(field_id=3, name="file_path", field_type=StringType(), required=False),
#         NestedField(field_id=4, name="pos", field_type=LongType(), required=False),
#         schema_id=0,
#     )
...
#     return Schema(
#         NestedField(field_id=1, name="number_partitioned", field_type=LongType(), required=False),
#         NestedField(field_id=2, name="primary_key1", field_type=StringType(), required=False),
#         NestedField(field_id=3, name="primary_key2", field_type=LongType(), required=False),
#         NestedField(field_id=4, name="primary_key3", field_type=StringType(), required=False),
#         NestedField(field_id=5, name="primary_key4", field_type=BooleanType(), required=False),
#         schema_id=0,
#     )
        ...
        if "primary_key2" in test_case["mock_data"][0]:
            ...
            if test_case["schema"] == "base_schema":
                ...
                for data in test_case["mock_data"]:
                    ...
                    run_spark_commands(
                        spark, [f"INSERT INTO {identifier} VALUES {values}"]
                    )
            else:
                # For other tests, just include the basic columns
                for data in test_case["mock_data"]:
                    values = ", ".join(
                        f"(0, {repr(pk)})" if pk is not None else "(0, NULL)"
...
TEST_CASES = [
    {
        ...
        "schema": "single_key_drop_duplicates_schema",
    ...
    },
    {
        ...
        "schema": "multi_key_drop_duplicates_schema",
    ...
    },
    {
        ...
        "schema": "position_delete_read_by_spark_schema",
        "identifier_fields": ["primary_key"],
        "mock_data": [
            {"primary_key": ["pk1", "pk2", "pk3"]},
            {"primary_key": ["pk1", "pk2", "pk3"]},
    ...
    {
        "name": "empty_data",
        "table_name": "table_converter_ray_empty_data",
        "schema": "empty_data_schema",
        "identifier_fields": ["primary_key"],
        "mock_data": [],  # Empty data
        "expected_result": {"primary_keys": []},
    },
    # None primary key
    {
        "name": "null_identifier_fields",
        "table_name": "table_converter_ray_null_identifiers",
        "schema": "null_identifier_fields_schema",
        "identifier_fields": ["primary_key"],
        "mock_data": [
            {"primary_key": ["pk1", None, "pk3"]},
            {"primary_key": [None, "pk2", "pk3"]},
        ],
        "expected_result": {"primary_keys": [None, "pk1", "pk2", "pk3"]},
    },
    {
        "name": "duplicate_keys_different_positions",
        "table_name": "table_converter_ray_dup_keys_diff_pos",
        "schema": "duplicate_keys_different_positions_schema",
        "identifier_fields": ["primary_key"],
        "mock_data": [
            {
                "primary_key": ["pk1", "pk2", "pk3"],
        ...
        "expected_result": {
            ...
            "pk_to_pos": {"pk1": 4, "pk2": 5, "pk3": 6},
        },
    },
    {
        "name": "many_identifier_fields",
        "table_name": "table_converter_ray_many_identifiers",
        "schema": "many_identifier_fields_schema",
        "identifier_fields": [
            "primary_key1",
            "primary_key2",
            "primary_key3",
        ...
        "mock_data": [
            {
                "primary_key1": ["pk1", "pk2"],
                "primary_key2": [1, 2],
                "primary_key3": ["a", "b"],
                "primary_key4": [True, False],
            }
        ],
        "expected_result": {
            "pk_tuples": [("pk1", 1, "a", True), ("pk2", 2, "b", False)]
        },
    },
    {
        "name": "mixed_case_primary_keys",
        "table_name": "table_converter_ray_mixed_case",
        "schema": "mixed_case_primary_keys_schema",
        "identifier_fields": ["primary_key"],
        "mock_data": [
            {"primary_key": ["PK1", "pk2", "Pk3"]},
            {"primary_key": ["pk1", "PK2", "pk3"]},
        ],
        "expected_result": {"primary_keys": ["PK1", "pk2", "Pk3", "pk1", "PK2", "pk3"]},
    },
]


# --- Schema Fixtures: One per test case ---
@pytest.fixture
def single_key_drop_duplicates_schema():
    return Schema(
        NestedField(
            field_id=1, name="number_partitioned", field_type=LongType(), required=False
        ),
        ...
        NestedField(
            field_id=3, name="file_path", field_type=StringType(), required=False
        ),
        NestedField(field_id=4, name="pos", field_type=LongType(), required=False),
        schema_id=0,
    )
...
@pytest.fixture
def multi_key_drop_duplicates_schema():
    return Schema(
        NestedField(
            field_id=1, name="number_partitioned", field_type=LongType(), required=False
        ),
...
@pytest.fixture
def position_delete_read_by_spark_schema():
    return Schema(
        NestedField(
            field_id=1, name="number_partitioned", field_type=LongType(), required=False
        ),
...
@pytest.fixture
def null_identifier_fields_schema():
    return Schema(
        NestedField(
            field_id=1, name="number_partitioned", field_type=LongType(), required=False
        ),
...
@pytest.fixture
def duplicate_keys_different_positions_schema():
    return Schema(
        NestedField(
            field_id=1, name="number_partitioned", field_type=LongType(), required=False
        ),
...
@pytest.fixture
def many_identifier_fields_schema():
    return Schema(
        NestedField(
            field_id=1, name="number_partitioned", field_type=LongType(), required=False
        ),
        ...
        NestedField(
            field_id=4, name="primary_key3", field_type=StringType(), required=False
        ),
        NestedField(
            field_id=5, name="primary_key4", field_type=BooleanType(), required=False
        ),
        schema_id=0,
    )
...
@pytest.fixture
def mixed_case_primary_keys_schema():
    return Schema(
        NestedField(
            field_id=1, name="number_partitioned", field_type=LongType(), required=False
        ),
