...
def collect_metastats(source_partition_locators: List[PartitionLocator],
    ...
    for partition_locator in source_partition_locators:
        partition_value = str(partition_locator.partition_values[1])
        partition_value_string = partition_value.replace(":", ".")
        ...
        stats_res_obj_ref_all_partitions[partition_id] = stats_res_obj_ref
    for partition_canonical_string, stats_res_obj_ref in stats_res_obj_ref_all_partitions.items():
        stats_res_all_partitions[partition_canonical_string] = ray.get(stats_res_obj_ref)
    ...
    for partition_val, delta_stream_range_set in stats_res_all_partitions.items():
        ...
        for stream_pos, statcolumnresult in delta_stream_range_set.items():
            for cs in statcolumnresult.column_stats[0].manifest_stats.stats:
                partition_stats_sum_row_count += cs.get("rowCount")

            for stats in statcolumnresult.get("column_stats"):
                partition_pyarrow_sum += stats.get("stats").get("pyarrowTableBytes")

        logger.info(f"partitions_stats_result: {partition_val}: rowCount: {partition_stats_sum_row_count}; pyarrowTableBytes: {partition_pyarrow_sum}")
...
def collect_from_partition(source_partition_locator: PartitionLocator,
                           ...
                           deltacat_storage=unimplemented_deltacat_storage,
                           *args,
                           **kwargs) -> Tuple[str, Dict[int, DeltaStats]]:
    ...
    stats_res_obj_ref = _start_all_stats_collection_from_deltas.remote(
                                ...
                                deltacat_storage)
    return source_partition_locator.partition_id, stats_res_obj_ref
...
def _start_stats_cluster(out_cluster_cfg: str,
    ...
    ray_up_letency = timed_invocation(
        func=ray_up,
        cluster_cfg=out_cluster_cfg
    )
    logger.info(f"ray_up_letency: {partition_val}:{ray_up_letency}")

    head_node_ip = get_head_node_ip(out_cluster_cfg)
    client = ray_init(head_node_ip, 10001)
    with client:
        delta_stream_range_stats, stats_collection_letency = timed_invocation(
    ...
    logger.info(f"actual_stats_collection_letency: {partition_val}: {stats_collection_letency}")
    client.disconnect()
