...
def _timed_merge(input: MergeInput) -> MergeResult:
    ...
    with memray.Tracker(
        ...
        hb_group_res = []
        for hb_idx, hb_res_ref in input.dfe_groups_refs.items():
            hb_group_hb_idx_res = input.object_store.get_many(hb_res_ref)
            hb_group_res.append(hb_group_hb_idx_res)
            input.object_store.delete_many(hb_res_ref)
        delta_file_envelope_groups_list = hb_group_res
        # object_store_stats = input.object_store.stats("10.0.89.221")
        # print(f"After_merge_deletes_hg_index{input.hash_group_index}: {object_store_stats}")
        # delta_file_envelope_groups_list = input.dfe_groups_refsfmerged_delta.meta.record_count

        # print(f"delta_file_envelope_groups_list:{delta_file_envelope_groups_list}")
        ...
        for delta_file_envelope_groups in delta_file_envelope_groups_list:
            ...
            # )
            # print(f"delta_file_envelope_groups:{delta_file_envelope_groups}")
            for hb_idx, dfes in enumerate(delta_file_envelope_groups):
                # print(f"hb_idx_merge: {hb_idx}")
                # print(f"dfes: {dfes}")
                if dfes:
