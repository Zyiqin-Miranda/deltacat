...
def _execute_compaction(
    ...
    # print(f"length_input_deltas:{len(input_deltas)}")
    # input_deltas = [input_deltas_list[0], input_deltas_list[1]]
    ...
    print(
        f"params.destination_partition_locator:{params.destination_partition_locator}"
    )
    print(f"compacted_stream_locator:{compacted_stream_locator}")
    ...
    uniform_delta_splits = 3
    split_index = len(uniform_delta_splits) // 3
    uniform_deltas_group = []
    uniform_deltas_group.append(uniform_deltas[:: split_index + 1])
    uniform_deltas_group.append(uniform_deltas[split_index + 1 : 2 * split_index + 1])
    uniform_deltas_group.append(uniform_deltas[2 * split_index + 1 : :])
    print(f"params:hash_group_count:{params.hash_group_count}")
    params.hash_group_count = 20
    print(f"OVERRIDE_HASH_GROUP_COUNT:{params.hash_group_count}")
    ...
    merge_res_list = []
    for hg_i in range(params.hash_group_count):
        print(f"hg_i:{hg_i}", flush=True)
        hb_results_generator_list = []
        object_ref_got_from_generator_list = []

        for hb_pending in hb_tasks_pending:
            next_obj = next(hb_pending)
            hb_results_generator_list.append(next_obj)
        object_ref_got_from_generator_list = ray.get(hb_results_generator_list)

        one_hash_group_idx_to_size_bytes = 0
        one_hash_group_idx_to_num_rows = 0
        res_list = defaultdict(list)
        for object_ref in object_ref_got_from_generator_list:
            for hb_idx, got_from_generator in enumerate(object_ref[1][0]):
                res_list[hb_idx].append(got_from_generator)

                print(
                    f"one_hash_group_idx_to_size_bytes: hash_group_idx:{hg_i}, {one_hash_group_idx_to_size_bytes}"
                )
                logger.info(
                    f"one_hash_group_idx_to_size_bytes: hash_group_idx:{hg_i}, {one_hash_group_idx_to_size_bytes}"
                )

                hg_index = object_ref_got_from_generator_list[0][0]
                stats_b = params.object_store.stats()
                print(f"Before_merge_object_store_stats:{stats_b}")
                logger.info(f"Before_merge_object_store_stats:{stats_b}")
                merge_input = MergeInput.of(
                    dfe_groups_refs=res_list,
                    write_to_partition=compacted_partition,
                    compacted_file_content_type=params.compacted_file_content_type,
                    primary_keys=params.primary_keys,
                    sort_keys=params.sort_keys,
                    merge_task_index=hg_index,
                    hash_bucket_count=params.hash_bucket_count,
                ...
                stats_a = params.object_store.stats()
                logger.info(f"After_merge_object_store_stats: {stats_a}")
        one_hash_group_idx_to_size_bytes += object_ref[1][1]
        one_hash_group_idx_to_num_rows += object_ref[1][2]
        merge_res_list.append(mg.merge(merge_input))
    ...
    print(f"length_materialize_res:{len(materialize_res)}")
    logger.info(f"length_materialize_res:{len(materialize_res)}")
    total_deduped_records = 0
    for mr in merge_res_list:
        total_deduped_records += mr.deduped_record_count
    logger.info(f"TOTAL_total_deduped_records:{total_deduped_records}")
    print(f"TOTAL_total_deduped_records:{total_deduped_records}")
    deltas = [mt.delta for mt in materialize_res]
    merged_delta = Delta.merge_deltas(
        deltas,
    )
    logger.info(f"merged_delta.meta.record_count:{merged_delta.meta.record_count}")
    print(f"merged_delta.meta.record_count:{merged_delta.meta.record_count}")
    ...
    #     num_hash_groups=params.hash_group_count,
    #     hash_group_size_bytes=one_hash_group_idx_to_size_bytes,
    #     hash_group_num_rows=one_hash_group_idx_to_num_rows,
    #     round_completion_info=round_completion_info,
    ...
    #             drop_duplicates=params.drop_duplicates,
    #             hash_group_index=hg_index,
    #             num_hash_groups=params.hash_group_count,_compact_tables
    #             max_records_per_output_file=params.records_per_compacted_file,
    #             enable_profiler=params.enable_profiler,
    #             metrics_config=params.metrics_config,
    #             s3_table_writer_kwargs=params.s3_table_writer_kwargs,
